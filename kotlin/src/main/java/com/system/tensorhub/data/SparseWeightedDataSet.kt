...
class SparseWeightedDataSet(dim: Dim, dataType: DataType, sparseDensity: Double) : SparseDataSet(dim, dataType, sparseDensity) {
    ...
    private fun putWeightOne(index: Int) {
        Arrays.fill(weights, index * getStride(), (index + 1) * getStride(), 1f)
    }
    ...
    /**
     * Adds sparse data with weights and updates the weights array.
     *
    ...
    // Reuse the existing overridden methods and remove duplicate code
